  ...
  _createClass(ReactHTMLTableToExcel, [{
    ...
    value: function handleDownload() {
      ...
      this.props.multipleTables.map((x) => {
        ...
        if (
          document.getElementById(x.tableId).nodeType !== 1 ||
          document.getElementById(x.tableId).nodeName !== 'TABLE'
        ) {
          if (process.env.NODE_ENV !== 'production') {
            console.error('Provided table property is not html table element');
          }
        ...
        console.log("multiTables",this.props.multipleTables);
        var table = document.getElementById(x.tableId).outerHTML;
        var sheet = String(this.props.sheet);
        var filename = String(x.fileName) + '.xls';
